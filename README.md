...
设置页支持独立教学模型、Web 导师、Brave 搜索及预算配置，专门的 `settings` 配对档案才能保存和显式测试连接。Key 仅进入服务器受保护文件；模型端点由操作者精确授权，CLI 配置不迁移。能力查询不发外部请求，研究与 Web 导师执行仍显示尚未实现。教学配置保存后需重启应用。详见[模型、搜索、授权与秘密生命周期](clients/web/README.md#模型搜索与预算设置)。
