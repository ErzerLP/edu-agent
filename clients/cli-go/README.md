...
非默认区资料、目标、教学依服务端能力开放；旧服务不支持时明确报错，不读取默认区数据冒充结果。总览和复习页面读取统一的服务端聚合，提供全局入口；目标详情可查看本目标进度及到期复习。Agent 区域绑定仍不在本外壳范围内。合法空结果、查询错误及投影延迟分别显示。
...
复习按稳定任务 ID 去重，同一节点在不同目标下的任务分开计算。可开始项进入原学习区和会话，再使用正式复习动作；原会话已完成、不在原节点或目标不可开始时会标明不可直接开始，页面不会迁移任务或替用户创建会话。服务升级后后台重建进度投影；重建尚未完成或失败时返回 `projection_unavailable`，不会显示“没有复习”。MCP 的 `learning.progress`、`learning.list_reviews` 支持同一范围和分页；客户端 Agent 使用全局进行中目标视图，并可按游标继续读取。
