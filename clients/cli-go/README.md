...
诊断阶段选择“生成路线预览”，可在工作台滚动查看学习意图、完成依据和资料节点；再选择“确认采用已展示路线”并按 `y` 才会应用。返回或刷新不采用提案，教学版本变化后必须重新预览。需要编辑规划时，可从目标或教学详情的 Agent 入口打开正式规划流程，也可使用 `goal plan`。目标详情启动的 Agent 会绑定该目标。贯通验证与平台边界见 [Issue #12 验收记录](../../docs/development/issue-12-acceptance.md)。
...
非默认区资料、目标、教学依服务端能力开放；旧服务不支持时明确报错，不读取默认区数据冒充结果。总览和复习页面读取统一的服务端聚合，提供全局入口；目标详情可查看本目标进度及到期复习。区内总览、目标和教学详情可打开对应绑定的 Agent 聊天，与教学续学入口分开。合法空结果、查询错误及投影延迟分别显示。

进度与复习也可独立查询：

```sh
edu-agent progress --global --limit 20
edu-agent --space UUID progress --goal-id GOAL_UUID --status all --json
edu-agent reviews --global --limit 20
edu-agent --space UUID reviews --goal-id GOAL_UUID --status all --json
edu-agent progress --global --status paused --order recent
```

默认仅展示进行中目标，排除归档学习区；`--status draft|paused|completed|archived|all` 显式查看其他状态，隐藏不删除历史或更改复习到期时间。`--all` 现在是全部目标状态的别名，不再由客户端拼装全库；继续分页使用返回的 `--cursor`，并保持原筛选条件。目标或学习区变化、投影代次切换时游标明确失效，应从首页重新读取。复习省略 `--due-before` 时由服务端冻结首次查询时间，后续页复用该截止时间。

路线显示完成步骤数、有效步骤总数和确认推进依据；没有路线时百分比未知。手动完成目标、证据支持的掌握状态和待确认评估分别展示。证据统计是同一目标的有效历史，JSON 保留原目标版本、资料版本和证据 ID；不把旧版本历史自动解释为满足当前目标。活跃时间沿用事件间隔估算，不是精确在线计时。

复习按稳定任务 ID 去重，同一节点在不同目标下的任务分开计算。可开始项进入原学习区和会话，再使用正式复习动作；原会话已完成、不在原节点或目标不可开始时会标明不可直接开始，页面不会迁移任务或替用户创建会话。服务升级后后台重建进度投影；重建尚未完成或失败时返回 `projection_unavailable`，不会显示“没有复习”。MCP 的 `learning.progress`、`learning.list_reviews` 支持同一范围和分页；客户端 Agent 使用绑定学习区及可选目标视图，可按游标继续读取，不静默回退全局。
...
空闲时 F7 选择学习区／目标／可选教学，优先恢复相同绑定的已有聊天，否则新建；原聊天不会被改标签。该入口结束原运行时，本地任务依原退出规则清理，no-save 会继续保持不保存。F2 默认按当前学习区及工作区过滤，Tab 可明确查看全部；跨区恢复仍使用原绑定和原工作区／端点确认，未完成批准不迁移。学习区名称与目标显示在 Agent 页，picker 显示绑定 ID 缩写；完整身份可通过正式选择页面核对。

旧历史统一映射固定默认区，不按标题猜测归属；旧历史可能包含过去的全局资料，此兼容映射不宣称旧内容已隔离。新绑定进入加密 SessionRecord v10、checkpoint v2 和服务端来源引用；不能把其他绑定的 checkpoint 安装到当前聊天。隐私代次变化会阻止旧正文继续送给回答、标题或压缩模型；恢复仍执行原正文隔离和授权重置。归档只读与失效写入由正式服务再次裁决。长期偏好继续使用现有全局合同，方向特定约束留在对应聊天，普通回答不是长期保存批准；不新增 Nocturne 分域存储。学习区不是 OS 沙箱，Shell 权限不变。Agent 导入入口使用 #6 预览／确认，本轮不接入可选 #7 后台任务恢复。
...
独立程序 `edu-companion` 供 Web 连接本机，不读取此 CLI 的配置、历史或 Keychain。
在仓库根执行 `make companion-build`，产物为 `clients/cli-go/bin/edu-companion`。
Linux 为本次原生测试环境；macOS 原生验收未完成，其他平台明确不支持。
此程序不会自动安装服务、后台自启或打开本地监听端口。

服务器操作者须同时显式启用 `WEB_UI_ENABLED=true`、`COMPANION_ENABLED=true`，
并正确配置 `PUBLIC_BASE_URL` 和 HTTPS。先在 Web 导师历史里打开一个具体对话，
选择“本地连接 → 开始配对本机”，然后在目标本机运行：

```bash
./clients/cli-go/bin/edu-companion --server https://你的学习站点 --workspace /你的工作区绝对路径
```

在提示中输入 Web 显示的一次性码，核对站点、设备、OS 用户与工作区，再输入“连接”。
不要把配对码放入命令参数、Shell history 或环境变量。回到 Web，核对终端与页面的设备身份相同，
分别选择文件、原生 Shell 和模型外发授权。学习登录和管理密码均不代替本机授权。
文件写入需要在 Web 阅读冻结预览并单独批准；版本变化拒绝发布。

Shell 使用此 OS 用户的原生权限、继承正常进程环境，没有新增命令白名单或工作区沙箱。
不要将文件授权误认为 Shell 的权限边界。文件、路径、命令、stdin、stdout/stderr/PTY 回显
会经学习服务器传输；只有另行允许模型外发后，导师才拥有本地工具。
授权绑定当前对话与模型目的地，不跨对话继承。参数字段的原始 command/env/input 不自动持久化；
明确外发的工具结果按导师对话的保存模式处理，回显可能包含私人内容。

Web 的“撤销连接并停止受管任务”、关闭/切换对话，或本机 Ctrl+C 都可结束授权。
正常退出收尾受管任务；网络断开使用 45 秒租约，考虑请求与清理等待最多约 65 秒完成有界检查。
无法证明清理完整时程序提示本机核查，不承诺终止已脱离进程组的进程。
刷新、服务重启或 companion 退出后须重新配对；本次输出只在内存，不能恢复或向旧 PID 发控制。
响应丢失后在 Web 查询原操作回执和任务，不重新执行命令或重发可能已写入的 stdin。

卸载时先撤销并退出程序，再移除自己安装的 `edu-companion` 二进制即可。
程序没有额外持久凭据、配置、守护进程或历史目录；不会删除 CLI 原有历史。
回环 HTTP 开发须由两端分别显式允许；本机程序使用
`--allow-loopback-http --server http://127.0.0.1:端口`，不接受非回环 HTTP。

协议、预算和验收缺口见 [设计](../../docs/design/local-companion.md) 与
[Issue #38 验收记录](../../docs/development/issue-38-acceptance.md)。
