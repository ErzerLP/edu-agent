...
func TestAgentUIEscapeStopsTurnPreservesVisibleWorkAndRejectsLateEvents(t *testing.T) {
	conversation := &cancellableActivityConversation{
		...
		activities: []agentloop.Activity{
			...
			{Kind: agentloop.ActivityReasoningDelta, Event: agentloop.Event{ID: "thinking-2"}, Delta: "cancelled-private-reasoning"},
	...
	for _, entry := range value.entries {
		...
		if entry.reasoning != nil && entry.reasoning.text.String() == "cancelled-private-reasoning" {
			retainedReasoning = true
		}
	}
	if !retainedReasoning || strings.Contains(view, "cancelled-private-reasoning") {
		t.Fatal("Esc lost partial reasoning or exposed it while folded")
	}
