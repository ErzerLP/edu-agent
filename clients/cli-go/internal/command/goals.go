...
func (a *App) runGoalManagement(ctx context.Context, args []string) error {
	if len(args) == 0 || args[0] == "help" || args[0] == "--help" {
		fmt.Fprintln(a.Out, "goal changes --id 目标ID [--change 变更ID] [--revision N] [--json]：读取教学变更状态、差异与补偿历史；支持 --space。")
		fmt.Fprintln(a.Out, "goal research|start-learning --goal UUID --input 请求.json：明确研究或空资料开学；详见 study help。保存目标本身仍不调用模型。")
