...
func (m *Manager) readOutput(ctx context.Context, owner, taskID, name string, offset int64, limit int) (OutputPage, error) {
	...
	// Bind cannot retire the backend while either the privacy authority or
	// saved segments are checked. Even a memory prefix or EOF needs the fence;
	// persistence may have failed before any metadata blob was saved.
	t.binding.mu.RLock()
	defer t.binding.mu.RUnlock()
	if err := m.checkOutputAccess(ctx, t); err != nil {
		return OutputPage{}, err
	}
	m.mu.Lock()
	...
	for count > 0 {
		...
		if readErr != nil {
			...
			if code == "output_unavailable" {
				m.mu.Unlock()
				return OutputPage{}, failure(code)
			}
			page = m.outputPageLocked(t, stream, offset)
...
// The binding read gate is held by the caller; no Manager lock spans I/O.
// Keep the last persistent authority after detach, so a revoked cache cannot
// become unrestricted memory. Tasks created memory-only need no such authority.
func (m *Manager) checkOutputAccess(ctx context.Context, t *task) error {
	m.mu.Lock()
	guarded := t.outputGuarded
	m.mu.Unlock()
	if !guarded {
		return nil
	}
	err := error(failure("output_unavailable"))
	if backend := t.binding.accessBackend; backend != nil {
		callCtx, cancel := context.WithTimeout(ctx, archiveCallLimit)
		err = backend.CheckArtifactAccess(callCtx)
		cancel()
	}
	if err == nil {
		return nil
	...
	if ctx.Err() != nil {
		return failure("search_canceled")
	}
