...
func TestWorkspaceDefinitionsExposeFileToolsWithoutShell(t *testing.T) {
	...
	if !reflect.DeepEqual(names, []string{ToolFind, ToolStat, ToolList, ToolRead, ToolSearch, ToolWrite, ToolEdit, ToolPatch, ToolMkdir, ToolCopy, ToolMove, ToolRestoreArchive, ToolPurgeArchive, ToolArchive}) {
		t.Fatalf("definitions=%+v", names)
	}
	for _, forbidden := range []string{"delete", "shell"} {
