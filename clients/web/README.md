...
设置页通过 `/v1/settings` 保存和恢复服务端配置，`/v1/capabilities` 只读查询能力，不发外部请求。教学模型、Web 导师模型及搜索分别配置；导师仅在明确选择时复用教学配置，不覆盖独立配置。CLI 本机 Key 不读取、不上传、不迁移。研究执行、Web 导师执行与 Web 作答仍明确显示未实现。
...
教学配置与预算保存后显示“需重启服务”，现有教学执行继续使用启动时冻结的配置；重启后从新配置构造真正的模型适配器。配置保存和探测立即可用。Web 导师配置留给后续运行消费，没有隐式切换现有教学、CLI 或 admin 设置。旧环境教学配置在未被显式编辑之前仍由环境拥有；保存搜索或预算不将环境 Key 迁移进文件。文件中的显式教学配置优先于环境起点；恢复环境管理需先停止服务，由操作者在备份后移走完整设置文件并重启，这也重置其他服务端设置，应谨慎执行。
...
预算使用整数并有服务端验证：研究请求默认 20（1–1000）、研究 Token 默认 100000（1024–10000000）、输出默认 2048（64–32768）、上下文默认 32768（4096–2000000）、无响应默认 60 秒（5–600）、并发默认 2（1–16）、存储默认 512 MiB（16–1048576）。输出须小于上下文且不超过研究 Token 预算。旧环境模型启动时使用其已配置的上下文和请求超时作为初值，仍需符合这些范围。当前教学为非流式请求；后续 SSE 必须以活动数据重置无响应计时，不能使用固定整次教学超时。后续运行在接受/续行时重新检查预算、当前用量、并发、存储和外发策略；这里不实现研究编排或统计计费，不限制固定教学轮数。未知提供商价格只显示请求用量和“估价未知”。

外部查询默认不允许私人目标、正文和成绩；显式开启后的策略供后续运行执行，不能代替任务授权。模型运行需要把相应学习上下文发送给所选模型端点；设置页在配置和测试前显示发送位置。本次没有新的私人查询运行。
