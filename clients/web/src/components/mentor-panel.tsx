...
export function MentorPanel({ goal, archivedSpace }: { goal: Goal; archivedSpace: boolean }) {
  const { session, prefix, drafts } = useIdentity()
  const query = useQueryClient()
  const key = JSON.stringify([...prefix, goal.learning_space_id, goal.goal_id, 'mentor'])
  ...
  }, [current.data, goal.goal_id, goal.learning_space_id, session.generation])
  useEffect(() => {
    if (!run) return
  ...
  const start = async () => {
    ...
    const payload = { session_id: run?.session_id ?? sessionId.current, expected_version: goal.revision, prompt, save, request_budget: requestBudget, token_budget: tokenBudget }
  ...
  return <section className="panel mentor-panel" aria-label="目标内导师">
    <h2>目标内导师</h2>
    <p className="hint">围绕这个目标交流、澄清想法。不会自动开学、修改目标或记录掌握度。</p>
    ...
    {!active && <form onSubmit={(e) => { e.preventDefault(); void start() }} onKeyDown={(e) => { if (e.key === 'Enter' && e.nativeEvent.isComposing) e.preventDefault() }}>
      <fieldset disabled={pending || inactive || !modelReady || !current.data || !session.capabilities.save_goal}>
        ...
        <p className="hint">目标和本次交流将发送到配置的模型端点：{configuration.data?.effective_mentor.endpoint}。关闭页面仅停止观察；需要终止请点击“停止运行”。</p>
