...
test('真实学习身份、预览差异、过期、未知结果核对和解除引用', async ({ page, browser }) => {
  ...
  await expect(page.getByText('已解决（resolved）', { exact: false }).first()).toBeVisible()
  await expect(page.getByRole('link', { name: '查看同一同步审阅' }).filter({ hasText: '查看同一同步审阅' }).first()).toBeVisible()
