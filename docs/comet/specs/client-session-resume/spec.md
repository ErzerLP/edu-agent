...
以下后续规格提供明确的本地能力扩展：`client-local-execution/spec.md` 允许 dirty marker 中的非执行任务意图；`client-local-output/spec.md` 允许由原 Session 密钥边界保护的独立加密输出产物；`client-file-patch/spec.md` 允许同一密钥与配额边界下的完整diff及逐文件结果产物；`client-recursive-copy/spec.md` 允许独立认证加密的复制清单、追加式分段结算及不可重放调用身份；`client-archive-restore/spec.md` 允许归档恢复的双端历史事实、非可执行WAL与同Session加密不可重放身份；`client-archive-purge/spec.md` 允许用户明确批准的永久归档清理根事实、独立加密完整清单/追加结算与不可重放身份。它们不允许原始命令/补丁/清理参数、env、stdin、输出、完整差异或完整复制/清理清单及日志正文进入稳定 checkpoint、transcript 与自动标题；这些产物只读恢复，不恢复候选、批准或执行器，原有业务 authority 和其他持久化禁令不变。
...
`edu-agent agent` 创建新 Session；零个 committed 用户轮次且没有回执、本地任务/调用身份或长结果证据的空 Session 在正常退出时不得污染历史列表。孤儿产物片段或无法完整列举的目录不能作为“没有数据”自动删除。`--no-save` 只影响当前新 Session，不改写持久默认。无参数 Dashboard 的“AI 学习助手”入口必须复用同一 dispatcher，并提供“新建 Session”和“恢复历史 Session”，不得复制 Session 加载或存储逻辑。
...
正常关闭在清理内存前尝试保存最新稳定 checkpoint；保存失败必须在 TUI 中可见并使 close/switch 路径诚实报告“最近内容未保存”。`Session.Close` 不能先清空 ledger 再导出。空 Session 只有在没有 committed 用户文本、没有副作用 receipt、没有人工标题且没有本地任务/调用身份/长结果（含孤儿片段）或目录读取错误时，正常关闭后才删除其临时 metadata。
