...
模型调用 `ask_user_question` 时，Agent 保存原 assistant tool-call 序列、暂停位置、turn ID 和已完成事件，不执行该调用后的兄弟工具，也不发起下一次模型请求。TUI 状态切换为“等待你的选择”，用独占底部面板替换普通 composer/footer，同时保留 transcript 可用 `PgUp/PgDn` 或鼠标滚轮查看。选项显示为 `1.`–`4.`；`↑/↓` 和数字键移动焦点，默认高亮不算提交。单选用 `Enter` 提交当前项；多选用 `Space` 勾选/取消，至少一项后 `Enter` 提交稳定顺序的 option ID 数组。“自定义输入”在单选中替代普通选项，在多选中可与普通选项并存，使用有界多行编辑器并以 `Shift+Enter` 换行。resize、滚动和模式切换不得丢失草稿、焦点或选择。
...
模型无响应超时由用户选择任意可表示的正时长，不设置 10 分钟或其他产品上限。上下文窗口与最大输出 tokens 也不设置人为上界，仅保留窗口至少 4096、输出正整数、类型及溢出校验；默认值不变，已有显式配置不自动改写。高配置值不等于模型能力协商，也不取消每次请求的窗口预算、协议正文防护和持久化配额。普通客户端请求超时与模型无响应超时是独立设置，前者不得缩短后者；持续接收数据的 SSE 仍按无响应而非整次总时长计时。

供应商返回的可读推理文本或摘要可以按 `client-live-reasoning` 合同通过 Ctrl+O 在当前 TUI 内存中展开；不展示加密、签名、redacted 或未知不透明结构，不推断接口未提供的内部思考。推理正文不记录到持久 transcript、不进入 checkpoint、Source/Observation/Reflection、自动标题、日志或长期记忆，也不回灌后续模型请求。仅当兼容端点要求在同一在途工具调用组中回传不透明 continuation 字段时，客户端可以在严格大小上限内暂存到该 turn 的私有协议状态，并在完成、取消或失败时销毁；此类不透明字段不是可读推理正文。
...
Agent TUI 不再设置独立顶部状态栏或全宽分隔线，transcript 直接使用可用页面顶部空间；产品标识在宽终端合并到右侧概览标题，在无侧栏的窄终端和普通主界面中由底部状态区承载。有界多行输入区按内容增长，底部状态与帮助区持续显示运行状态、估算上下文、模型、推理强度和响应式键位提示。普通对话状态下，鼠标滚轮、`↑/↓` 与 `PgUp/PgDn` 都滚动 transcript；向上离开底部时暂停自动跟随并保留“有新消息”提示，滚回底部后自动恢复跟随。选择面板激活时 `↑/↓` 继续专用于选项焦点，`PgUp/PgDn` 和鼠标滚轮仍滚动 transcript。宽终端在右侧显示有界学习状态栏：Agent 区显示公开运行状态，学习区通过既有认证 `CurrentSession` 读取权威目标、会话状态、路线位置、当前 Activity 和估算活跃时间；启动、完整 turn 结束或显式刷新时重新读取，失败不阻断对话，窄终端则完全折叠且不缩窄 transcript 到合同下限以下。侧栏不显示 opaque ID、凭据、隐藏推理或原始工具参数，不持久化学习副本，也不把展示快照注入模型上下文。压缩、降级与来源不可用事件仍以不含正文的结构化 transcript 卡片呈现。

`Ctrl+O` 是统一执行详情与实时推理展示开关。收起时只显示简洁的思考阶段和工具摘要；展开时显示客户端可证明的阶段名称、运行/完成/停止/失败状态、经过时间、最近响应活动、工具显示名、有界进度和稳定错误码，以及接口实际返回且仍在内存保留的可读推理正文。没有正文时明确说明，不伪造思考链；正文保留与隐私边界见 `client-live-reasoning`。它不显示工具参数、检索原文、opaque activity ID、凭据或原始供应商错误正文。传统终端无法可靠识别单独 Ctrl，也没有可靠独立 `Ctrl+0` 编码，因此正式帮助与验收只承诺 `Ctrl+O`。

模型请求、上下文准备、SSE 接收、工具组装、工具执行、响应校验和工具结果后的继续分析均有真实阶段状态。一次前台模型请求的这些阶段必须原地更新同一活动身份；工具执行后的“继续分析”属于下一次请求，不得另建不会结算的运行中卡片。已经完成的前次请求仍保留独立完成记录；失败、超时、取消或轮数保护停止后不得遗留额外的“思考中”。运行中使用有界 tick 原地更新时间，不持续追加重复卡片；达到慢响应阈值时显示已运行时间、当前无响应超时配置和 `Esc` 停止提示，不能把无响应 timeout 误示为整轮总预算，也不能把静态 spinner 当作唯一反馈。网络、SSH 或供应商仍可能慢，但用户可以区分正在等待、正在接收、正在执行工具、正在停止和已经失败。
