...
不再设置独立顶部状态栏或全宽分隔线，transcript 直接占用页面顶部空间。产品标识在宽终端进入右侧栏标题，在无侧栏的窄终端进入底部状态行，避免重复占用垂直空间：
...
```text
◇ edu-agent · AI 学习助手 · ● 就绪 · 上下文约 54% · 模型 deepseek-chat
滚轮/↑/↓/PgUp/PgDn 历史 · Enter 发送 · Ctrl+J 换行 · Esc 退出
```

普通 composer 状态下，`↑/↓` 每次滚动一行，`PgUp/PgDn` 按页滚动，鼠标滚轮按有界步长滚动；原 `Ctrl+↑/↓` 快速滚动和 `Ctrl+G` 到底部继续兼容。向上离开底部后暂停自动跟随，新内容只更新“有新消息”提示而不抢回视口；滚动到底部后恢复自动跟随。选择器激活时 `↑/↓` 仍用于选项焦点，`PgUp/PgDn` 与鼠标滚轮继续查看 transcript。

产品标识、运行状态、上下文和模型元数据全部位于侧栏或底部，不再占用独立顶部行。token 为估算时显示“约”，避免制造精确错觉。窄终端按“最近轮次 → 记忆条数 → 次要键位”的顺序降级，但必须保留当前状态、模型和发送/退出提示；滚动帮助至少保留方向键短提示。新消息提示属于动态状态行，不插入 transcript。

视觉原则参考 Crush 在固定 commit `7944b8e52225d8805e31eacbf7ef24856b0dfb7a` 的实现，但不复制其品牌或完整架构：采用无独立顶部栏、动态高度 textarea、语义颜色、底部 status/help 分层、响应式裁剪和有界右侧栏；本项目继续使用现有 Bubble Tea/viewport 架构，不引入 Ultraviolet 或 Crush 的多会话/文件侧栏状态机。
...
```text
<conversation transcript>                 ╭─ ◇ edu-agent ────────╮
                                          │ AGENT              │
╭─ 消息 ───────────────────────────────╮  │ ● 就绪             │
