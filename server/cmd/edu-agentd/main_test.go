...
func TestPairingCodeProfileParsing(t *testing.T) {
	tests := []struct {
		...
		{name: "显式浏览器导入权限", args: []string{"pairing-code", "create", "--profile", "import"}, profile: identity.PairingProfileImport},
		{name: "显式浏览器评估审批权限", args: []string{"pairing-code", "create", "--profile", "assessment"}, profile: identity.PairingProfileAssessment},
