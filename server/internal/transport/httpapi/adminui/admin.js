(() => {
  ...
  const statusMeta = Object.freeze({
    ...
    in_sync: ["已同步", "success"],
    remote_unchanged: ["远端未变化", "success"],
    resolved: ["已解决", "success"],
    closed: ["已关闭", "success"],
    absence_verified: ["已确认不存在", "success"],
    ...
    not_configured: ["未配置", "warning"],
    open: ["待审阅", "warning"],
    partial: ["部分完成", "warning"],
    not_applicable: ["不适用", "neutral"],
    unsupported: ["不支持", "warning"],
    local_changed: ["本地已变化", "warning"],
    remote_changed: ["远端已变化", "warning"],
    ...
    redacted: ["已清除", "danger"],
    revoked: ["已撤销", "danger"],
    incompatible: ["不兼容", "danger"],
    not_ready: ["未就绪", "danger"],
    superseded: ["已被替代", "neutral"],
    delete_pending: ["等待删除", "warning"],
    deleted: ["已删除", "danger"],
    fenced: ["已隔离", "danger"],
    expiry_reconciling: ["正在核对过期状态", "warning"],
    expired: ["已过期", "danger"],
    both_changed: ["两端均已变化", "danger"],
    remote_moved: ["远端身份已移动", "danger"],
    unbased_remote: ["远端文档未纳管", "warning"],
    path_occupied: ["路径已占用", "danger"],
    invalid_remote_markdown: ["远端文档无效", "danger"],
    prepared: ["已准备", "warning"],
    sent: ["已发送", "warning"],
    reconciling: ["正在核对", "warning"],
  ...
  const reasonLabel = (value) => {
    ...
    return (
      {
        local_revision_changed: "本地知识修订已变化",
        both_sides_changed: "本地与远端均已变化",
        remote_identity_moved: "远端文档身份已移动",
        unmanaged_remote_note: "远端文档尚未纳管",
        remote_markdown_invalid: "远端 Markdown 无效",
        remote_content_changed: "远端内容已变化",
        remote_note_missing: "远端文档缺失",
        remote_path_occupied: "远端路径已占用",
        publication_preflight_changed: "发布前状态已变化",
  ...
  function scheduleSessionExpiry(value) {
    ...
    const delay = Math.max(
      0,
      Math.min(2_147_000_000, expiresAt - Date.now() + 250),
    );
  ...
  async function bootstrapConsole() {
    try {
      ...
      const page = validPages.has(location.hash.slice(1))
  ...
  function renderOverview() {
    ...
    header.textContent =
      readiness.status === "healthy"
        ...
        : readiness.status === "not_ready"
          ? "服务未就绪"
          : "服务降级";
    byId("sidebarStatus").textContent = ready
      ? `服务器已连接 · ${statusLabel(readiness.status)}`
      : "服务器未就绪";

    const componentEntries =
      readiness.components && !Array.isArray(readiness.components)
        ? Object.entries(readiness.components)
        : [];
    const services = Array.isArray(readiness.components)
      ? readiness.components
      : componentEntries.map(([name, component]) => ({ name, ...component }));
    const memoryComponent =
      services.find((item) => item.name === "nocturne") || {};
    const notesyncComponent =
      services.find((item) => item.name === "notesync") || {};
    const stripValues = [
    ...
    byId("statusStrip").replaceChildren(
      ...stripValues.map(([label, value]) => {
    ...
    if (services.length) {
      services.forEach((service) => {
        ...
        name.append(
          text("strong", componentLabel(service.name)),
          text(
            "span",
            ["model", "notesync", "open_evaluation_worker"].includes(
              service.name,
            )
              ? "可选集成"
              : "核心服务",
        ...
        const badge = text(
          ...
          `status-badge ${statusClass(service.status)}`,
        );
        row.append(
          name,
          badge,
          text("span", service.reason || "运行正常", "service-reason"),
        );
  ...
  function deviceTableRow(device) {
    ...
    name.append(
      text("strong", device.display_name || "未命名设备"),
      text("span", device.id || "-"),
    );
    ...
    status.append(
      text(
        "span",
        device.revoked_at ? "已撤销" : "有效",
        `status-badge ${device.revoked_at ? "danger" : "success"}`,
      ),
    );
    const used = text(
      "td",
    ...
    if (!device.revoked_at)
      actions.append(
        button("撤销", "secondary-button", () => revokeDevice(device)),
      );
  ...
  function deviceCard(device) {
    ...
    [
      ["权限", (device.scopes || []).join("、") || "-"],
      ["最近使用", formatDate(device.last_used_at || device.created_at)],
    ].forEach(([k, v]) => dl.append(text("dt", k), text("dd", v)));
    card.append(head, dl);
    if (!device.revoked_at)
      card.append(
        button("撤销设备", "secondary-button", () => revokeDevice(device)),
      );
    return card;
  }
  ...
  function scopeList(scopes) {
    ...
    scopes.forEach((scope) =>
      wrapper.append(text("span", scope, "scope-chip")),
    );
    return wrapper;
  }
  ...
  async function revokeDevice(device) {
    const confirmed = await confirmAction(
      "撤销设备",
      `撤销“${device.display_name || shortID(device.id)}”后，该设备的现有凭据将失效。`,
    );
    if (!confirmed) return;
    try {
  ...
  async function loadMoreMemory() {
    ...
    try {
      const page = await api(
        `/admin/api/memory?limit=200&cursor=${encodeURIComponent(cursor)}`,
      );
      state.memory = {
        ...page,
        items: [...(state.memory.items || []), ...(page.items || [])],
      };
      renderMemory();
    } catch (error) {
      showNotice(error.message, "danger");
  ...
  function renderMemory(focusID = "") {
    const items = state.memory?.items || [];
    const active = items.filter(
      (item) => item.record?.status === "applied",
    ).length;
    const queued = items.filter(
      (item) => item.delivery_status === "queued",
    ).length;
    const unavailable = items.filter(
      (item) => item.content_status !== "available",
    ).length;
    ...
    const filtered = items.filter((item) => {
      ...
      return `${item.content || ""} ${record.logical_memory_id || ""} ${record.record_revision_id || ""}`
        .toLowerCase()
        .includes(query);
    });
    ...
    if (!filtered.length) {
      tree.append(
        empty(
          "没有记忆节点",
          query ? "没有匹配当前筛选的内容。" : "服务器尚未导出记忆内容。",
        ),
      );
      byId("memoryDetail").replaceChildren(
        empty("选择记忆节点", "查看内容、版本与 Nocturne 投递状态。"),
      );
      return;
    }
    const selected =
      filtered.find(
        (item) => item.record?.record_revision_id === state.selectedMemory,
      ) || filtered[0];
    state.selectedMemory = selected.record?.record_revision_id || "";
    const groups = [
      [
        "当前记忆",
        filtered.filter((item) => item.record?.status === "applied"),
      ],
      [
        "历史与队列",
        filtered.filter((item) => item.record?.status !== "applied"),
    ...
    groups.forEach(([label, members]) => {
      ...
      members.forEach((item) => {
        const record = item.record || {};
        const recordID = record.record_revision_id || "";
        const title =
          firstMeaningfulLine(item.content) ||
          `记忆 ${shortID(record.logical_memory_id)}`;
        const node = document.createElement("button");
        node.type = "button";
        node.dataset.memoryId = recordID;
        node.className = `tree-node${state.selectedMemory === recordID ? " active" : ""}`;
        node.setAttribute(
          "aria-pressed",
          String(state.selectedMemory === recordID),
        );
        ...
        main.append(
          ...
          text(
            "span",
            `${shortID(record.logical_memory_id)} · 版本 ${record.revision || 0}`,
          ),
        );
        node.append(
          main,
          text(
            ...
            `tree-node-meta status-badge ${statusClass(item.delivery_status)}`,
          ),
        );
        node.addEventListener("click", () => {
          state.selectedMemory = recordID;
          renderMemory(recordID);
        });
        tree.append(node);
      });
    });
    ...
    if (focusID) {
      const focusTarget = [
        ...tree.querySelectorAll("button[data-memory-id]"),
      ].find((candidate) => candidate.dataset.memoryId === focusID);
      focusTarget?.focus();
    }
  }
  ...
  function renderMemoryDetail(item) {
    ...
    titleWrap.append(
      text("p", "MEMORY NODE", "section-kicker"),
      text(
        "h2",
        firstMeaningfulLine(item.content) ||
          `记忆 ${shortID(record.logical_memory_id)}`,
    ...
    heading.append(
      ...
      text(
        ...
        `status-badge ${statusClass(record.status)}`,
      ),
    );
    ...
    [
      ...
      [
        "回执",
        item.receipt?.status ? statusLabel(item.receipt.status) : "-",
        item.receipt?.status || "",
      ],
    ].forEach(([key, value, status]) => {
      const fact = document.createElement("div");
      fact.className = "detail-fact";
      const valueNode = status
        ? text("strong", value, `status-badge ${statusClass(status)}`)
        : text("strong", value || "-");
    ...
    content.append(
      text("h3", "记忆内容"),
      text("pre", item.content || "内容当前不可用。", "content-preview"),
    );
  ...
  function renderKnowledge(focusKey = "") {
    const data = state.knowledge || {};
    const documents = data.export?.documents || [];
    const nodes =
      data.tree?.revision?.documents?.flatMap(
        (document) => document.document?.nodes || [],
      ) || [];
    renderMetrics("knowledgeStats", [
      ["当前修订", data.head?.revision_no || 0],
      ["文档", documents.length],
      ["知识节点", nodes.length],
      ["更新时间", data.head ? formatDate(data.head.created_at) : "-"],
    ]);
    const query = byId("knowledgeSearch").value.trim().toLowerCase();
    const filtered = documents.filter((document) =>
      `${document.path} ${document.markdown}`.toLowerCase().includes(query),
    );
    ...
    const selected =
      filtered.find(
        (document) => document.path === state.selectedKnowledgePath,
      ) || filtered[0];
    state.selectedKnowledgePath = selected.path;
    const selectedTree = data.tree?.revision?.documents?.find(
      (entry) => entry.path === selected.path,
    );
    const selectedNode = selectedTree?.document?.nodes?.find(
      (entry) => entry.node_revision_id === state.selectedKnowledgeNode,
    );
    if (!selectedNode) state.selectedKnowledgeNode = "";
    filtered.forEach((knowledgeDocument) => {
      const documentTree = data.tree?.revision?.documents?.find(
        (entry) => entry.path === knowledgeDocument.path,
      );
      const nodeCount = documentTree?.document?.nodes?.length || 0;
      const node = document.createElement("button");
      node.type = "button";
      const documentFocusKey = `document:${knowledgeDocument.path}`;
      node.dataset.knowledgeFocus = documentFocusKey;
      ...
      main.className = "tree-node-main";
      main.append(
        text("strong", knowledgeDocument.path),
        text("span", `${nodeCount} 个结构节点`),
      );
      node.append(
        main,
        text(
          "span",
          `${byteSize(knowledgeDocument.markdown)} KB`,
          "tree-node-meta",
        ),
      );
      node.addEventListener("click", () => {
        state.selectedKnowledgePath = knowledgeDocument.path;
        state.selectedKnowledgeNode = "";
        renderKnowledge(documentFocusKey);
      });
      tree.append(node);
      if (documentTree?.document?.nodes?.length) {
        documentTree.document.nodes
          .filter((entry) => entry.title)
          .slice(0, 80)
          .forEach((entry) => {
            const child = document.createElement("button");
            child.type = "button";
            const childFocusKey = `node:${knowledgeDocument.path}:${entry.node_revision_id}`;
            child.dataset.knowledgeFocus = childFocusKey;
            const childActive =
              state.selectedKnowledgePath === knowledgeDocument.path &&
            ...
            childMain.append(
              ...
              text("span", `第 ${entry.heading_range?.start_line || 0} 行`),
            );
            child.append(childMain);
            child.addEventListener("click", () => {
              state.selectedKnowledgePath = knowledgeDocument.path;
              state.selectedKnowledgeNode = entry.node_revision_id;
              renderKnowledge(childFocusKey);
            });
            tree.append(child);
          });
      }
    });
    byId("knowledgeDetailTitle").textContent = selectedNode
      ? `${selected.path} · ${selectedNode.title}`
      : selected.path;
    byId("knowledgeContent").textContent = selectedNode
      ? markdownSection(selected.markdown, selectedNode.section_range)
      : selected.markdown || "";
    byId("downloadKnowledge").disabled = false;
    if (focusKey) {
      const focusTarget = [
        ...tree.querySelectorAll("button[data-knowledge-focus]"),
      ].find((candidate) => candidate.dataset.knowledgeFocus === focusKey);
      focusTarget?.focus();
    }
  }

  function downloadSelectedKnowledge() {
    const documentData = state.knowledge?.export?.documents?.find(
      (entry) => entry.path === state.selectedKnowledgePath,
    );
  ...
  async function loadMoreNotesyncReviews() {
    ...
    try {
      const page = await api(
        `/admin/api/notesync/reviews?cursor=${encodeURIComponent(cursor)}`,
      );
      state.notesync.reviews = [
        ...(state.notesync.reviews || []),
        ...(page.items || []),
      ];
  ...
  function renderNotesync() {
    ...
    const reviewCount = data.reviewsNextCursor
      ? `${(data.reviews || []).length}+`
      : (data.reviews || []).length;
    renderMetrics("notesyncStats", [
      [
        "运行状态",
        runtime.configured
          ? runtime.compatible
      ...
      ["版本", runtime.version || "-"],
      ["Vault", runtime.vault || saved.vault || "-"],
      ["待审阅", data.reviewsError ? "加载失败" : reviewCount],
    ]);
    byId("notesyncEnabled").checked = Boolean(saved.enabled);
    byId("notesyncBaseURL").value = saved.base_url || "";
    byId("notesyncToken").value = "";
    byId("notesyncToken").placeholder = saved.api_key_configured
      ? "已保存，留空保持不变"
      : "输入 API 密钥";
    ...
    [
      ...
      "notesyncPrefix",
      "notesyncSave",
    ].forEach((id) => (byId(id).disabled = !data.settings_writable));
    ...
    saveState.textContent = data.restart_required
      ? "等待重启"
      : data.settings_writable
        ? "已生效"
        : "只读";
    if (data.restart_required)
      showNotice("NoteSync 配置已保存，重启服务器后生效。", "warning");
  ...
  async function loadNotesyncPreviewPage(page, append) {
    ...
    control.disabled = true;
    const path = append
      ? state.notesync?.previewPath || ""
      : byId("notesyncPreviewPath").value.trim();
    try {
      ...
      state.notesync.preview = {
        ...preview,
        items: [...priorItems, ...(preview.items || [])],
      };
  ...
  function renderNotesyncPreview(preview) {
    ...
    if (!items.length) {
      host.append(
        empty("没有同步差异", "当前路径没有需要处理的 NoteSync 变化。"),
      );
      return;
    }
    items.forEach((item) =>
  ...
  function renderNotesyncReviews(reviews, errorMessage) {
    ...
    byId("loadMoreNotesyncReviews").hidden =
      Boolean(errorMessage) || !state.notesync?.reviewsNextCursor;
    host.replaceChildren();
    if (errorMessage) {
  ...
  function compactItem(
    ...
    if (secondaryLabel)
      badges.append(
        text("span", secondaryLabel, `status-badge ${secondaryKind}`),
      );
    ...
    return (
      {
        ...
        model: "教学模型",
        open_evaluation_worker: "评估 Worker",
        offline_signer: "离线签名",
        offline_protocol: "离线协议",
        nocturne: "Nocturne Memory",
        notesync: "NoteSync",
      }[value] ||
      value ||
      "未知组件"
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    byId("refreshPage").addEventListener("click", () =>
      loadPage(state.activePage, true),
    );
    ...
    byId("loadMoreNotesyncPreview").addEventListener(
      "click",
      loadMoreNotesyncPreview,
    );
    byId("loadMoreNotesyncReviews").addEventListener(
      "click",
      loadMoreNotesyncReviews,
    );
    byId("logoutButton").addEventListener("click", logout);
    byId("mobileLogoutButton").addEventListener("click", logout);
    window.addEventListener("hashchange", () => {
      const page = location.hash.slice(1);
      if (validPages.has(page) && page !== state.activePage)
        navigate(page, true);
    });
